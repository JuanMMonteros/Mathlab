...
h1 = sinc(nline-delay);
ntaps=257; % Mantenerlo impar para simplificar
group_delay = (ntaps-1)/2; % Este delay hay que compensarlo porque representa la cola de la convolucion
nline = -(ntaps-1)/2:(ntaps-1)/2';
h2 = sinc(nline-delay);
h3= my_rcosine(1,1,0.1,ntaps,delay);
h4=fir1(ntaps, .99);
tone_dig1=transpose(tone_dig);
tone_dig1=[tone_dig1; zeros(group_delay,1)];
tone_dig_delay = filter(h1,1,tone_dig1);% Agrego zeros para mantener el largo de la senial filtrada
...
sl=1:NFFT/2;
H1 = fftshift(fft(h1,NFFT)); % Filtro
H2 = fftshift(fft(h2,NFFT));
H3 = fftshift(fft(h3,NFFT));
H4 = fftshift(fft(h4,NFFT));
p1 = plot(f_v/1e6,abs(H1),'-r','Linewidth',2);
p1.MarkerFaceColor = p1.Color;
p1.MarkerEdgeColor = p1.Color;
hold all
plot(f_v/1e6,abs(H2),'-b','Linewidth',2);
plot(f_v/1e6,0.9*abs(H3),'--k','Linewidth',2);
plot(f_v/1e6,0.8*abs(H4),'-.c','Linewidth',2);
xlim([-201, 201]);
ylim([0,1.3]);
tit = sprintf('Respuesta de los distintos filtros');
title(tit,'Interpreter','latex','FontSize', fz);
xlabel('Frequency [MHz]', 'Interpreter','latex','FontSize', fz);
ylabel('Amplitude', 'Interpreter','latex','FontSize', fz);
legend({'Sinc','Sinc-mas taps','Coseno Realsado','Filtro Fir', }, 'Interpreter','latex','FontSize', fz-2);
grid on;
set(gcf, 'Position', [550 50 800 500],'Color', 'w');

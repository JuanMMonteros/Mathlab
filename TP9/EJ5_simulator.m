...
if exist('config_s','var')
    ...
    max_range = 1.5e3;  % Distancia maxima alcanzable [m]
...
% figure
% plot(t_tx,abs(s_t).^2)
% title('Pot de senal transmitida')
...
for niter = 1:Niters
    ...
    if niter==1 & plot_on
        ...
        title('Grafico 1: Salida del MF')
        xlabel('Cantidad de Muestras')
        %xlim([max(abs(y_mf))-10 , max(abs(y_mf))+10]);
        %ylim([0,3e-4])
        ylabel('Amplitud')
        legend('Salida del MF')
        hold all
        ...
        plot(timeline, abs(y_mf).^2); hold all;
        timeline2 = 1/(fs)*(phase_decim:NOS:length(y_mf)-1);
        plot(timeline2, y_mf_decim_sq,'o', 'MarkerSize', 8, 'MarkerEdgeColor', 'r', 'MarkerFaceColor', 'r');
        grid on
        
        title('Grafico 2: Salida del MF')
        xlabel('Tiempo [seg]')
        %xlim([range+10*deltaR , range+10*deltaR])
        %ylim([0,5e-4])
        ylabel('Amplitud')
        legend('Salida del MF', 'muestras')
        
        
        
        subplot(313);
        timeline2 = 1/(fs)*(phase_decim:NOS:length(y_mf)-1)*c*0.5;
        plot(timeline2, y_mf_decim_sq,'o', 'MarkerSize', 8, 'MarkerEdgeColor', 'r', 'MarkerFaceColor', 'r');
        grid on
       
        title('Grafico 3: Salida del MF')
        xlabel('Rango al target [m]')
        %xlim([range+10*deltaR , range+10*deltaR])
        %ylim([0,5e-4])
        ylabel('Amplitud')
        legend('Salida del MF')
